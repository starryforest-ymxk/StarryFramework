...
namespace StarryFramework
{
    internal class AudioManager : IManager
    {
        ...
        void IManager.Init()
        {
            ...
            clearTimer.Interval = FrameworkManager.setting.AudioSettings.clearUnusedAudioInterval;
            PreloadBankData(FrameworkManager.setting.AudioSettings.globalBanks);
        }
        ...
        {
            UnloadBankData(FrameworkManager.setting.AudioSettings.globalBanks);
        ...
        private void StopUnnamedEvent(GUID guid, FMOD.Studio.STOP_MODE mode)
        {
            if (!unnamedEventDic.ContainsKey(guid))
            {
                UnityEngine.Debug.LogWarning($"Audio event [{guid}] not found.");
                return;
            }
            else
            {
        ...
        private EventInstance GetNamedEventInstance(string tag)
        {
            ...
            {
                UnityEngine.Debug.LogError($"Audio event with tag [{tag}] not found.");
...
;           }
        ...
        private void AddNamedEventInstance(string tag, EventInstance eventInstance)
        {
            if(namedEventDic.ContainsKey(tag))
            {
                UnityEngine.Debug.LogError($"Audio event with tag [{tag}] has already existed.");
                return;
            }
            else
            {
        ...
        private void RemoveNamedEventInstance(string tag)
        {
            if (!namedEventDic.ContainsKey(tag))
            {
                UnityEngine.Debug.LogError($"Audio event with tag [{tag}] doesn't exist.");
                return;
            }
            else
            {
        ...
        internal void PlayBGM(EventReference _event)
        {
            ...
            {
                UnityEngine.Debug.LogWarning("BGM is playing.");
            }
        }
        ...
        internal void SetBGMPause(bool value)
        {
            ...
            {
                UnityEngine.Debug.LogWarning("BGM has stopped.");
            }
        }
        ...
        internal void SetVolume(string VCAPath, float value)
        {
            ...
            {
                UnityEngine.Debug.LogError("invalid volume value");
            }
