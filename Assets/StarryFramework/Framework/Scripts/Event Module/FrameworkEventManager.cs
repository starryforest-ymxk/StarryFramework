...
namespace StarryFramework
{
    ...
    internal class FrameworkEventManager 
    {
        ...
        internal void RemoveEventListener(string eventName, UnityAction action)
        {
            ...
            {
                Debug.LogWarning($"Framework Event Manager : 尝试删除不存在的事件[{eventName}]");
            }
        }
        ...
        internal void InvokeEvent(string eventName)
        {
            ...
            {
                Debug.Log($"Framework Event Manager : 尝试触发不存在的事件[{eventName}]");
            }

            if(FrameworkManager.setting.ModuleInUse(ModuleType.Event) && FrameworkManager.setting.EventSettings.InternalEventTrigger)
            {
        ...
        internal void ClearEventLinstener(string eventName)
        {
            ...
            {
                Debug.LogWarning($"Framework Event Manager : 尝试清空不存在的事件[{eventName}]");
            }
        }
