...
namespace StarryFramework
{
    ...
    public class FSM<T> : FSMBase, IFSM<T> where T : class
    {
        ...
        private FSM(string name, T owner, List<FSMState<T>> list)
        {
            ...
            if (name == null)
            {
                Debug.LogError("Name of FSM can not be null.");
                return;
            }
            ...
            {
                Debug.LogError("FSM list is invalid.");
                return;
            }
            ...
            foreach (FSMState<T> state in list)
            {
                ...
                {
                    Debug.LogError($"FSM '{name}' state '{type}' is already exist.");
                    continue;
                }
        ...
        public bool HasState(Type stateType)
        {
            if(stateType == null)
            {
                Debug.LogWarning("TimerState type is null");
                return false;
            }
            else
            {
        ...
        public FSMState<T> GetState(Type stateType)
        {
            if (stateType == null)
            {
                Debug.LogError("TimerState type can't be null");
                return null;
            }
            else if (!_states.ContainsKey(stateType))
            {
                Debug.LogError($"FSM {Name} doesn 't have state {stateType}.");
                return null;
            }
            else 
        ...
        public FSMState<T> GetCurrentState()
        {
            ...
            {
                Debug.Log($"FSM {Name} has not started yet.");
                return null;
            }
        }
        ...
        public bool HasData(string key)
        {
            if(key == null)
            {
                Debug.LogError("key can't be null.");
                return false;
            }
            else
        ...
        public D GetData<D>(string key)
        {
            ...
            {
                Debug.LogError("FSM Doesn't contain variable.");
                return default;
            }
            else
            {
                ...
                {
                    Debug.LogError($"the type of variable is not \"{typeof(D)}\".");
                    return default;
                }
                else
        ...
        public void RemoveData(string key)
        {
            ...
            {
                Debug.LogWarning("FSM Doesn't contain variable.");
            }
            else
            {
                _data.Remove(key);
        ...
        public void Start(Type stateType)
        {
            if (_isRunning)
            {
                Debug.LogWarning($"FSM {Name} is running, can not start again.");
            }

            if (stateType == null)
            {
                Debug.LogError("TimerState type can not be null.");
            }

            if (!_states.ContainsKey(stateType))
            {
                Debug.LogError($"TimerState {stateType} is invalid.");
                return;
            }
        ...
        public void ChangeState(Type stateType)
        {
            if (_isDestroyed)
            {
                Debug.LogError($"FSM {Name} has been released.");
                return;
            }
            if(!_isRunning)
            {
                Debug.LogError($"FSM {Name} has not started.");
                return;
            }
            if(stateType == null) 
            {
